// Recepção de Dados do Formulario
function getParameters() {
    ...
    if (paramsStart != -1) {
        ...
        for (var i = 0; i < params.length; i++) {
            ...
            pairArray[1] = pairArray[1].replace(/\+/g, ' '); ; // Substitui + por espaço em branco
        ...
        for(var key in paramsRet) {
            if(document.getElementById(key) != null) {
                document.getElementById(key).value = paramsRet[key];
            }
        }
...
function toggleModal(){
  if(document.querySelector('.contact-modal').style.display == 'flex')
